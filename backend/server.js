  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/contact') {
      ...
      if (!name || name.length < 2 || name.length > 120) {
        return send(res, 400, { ok: false, error: 'Nome inválido.' });
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254) {
        return send(res, 400, { ok: false, error: 'E-mail inválido.' });
      }
      if (!message || message.length < 5 || message.length > 5000) {
        return send(res, 400, { ok: false, error: 'Mensagem inválida.' });
      }
    ...
    console.error(error);
    return send(res, error.message === 'Payload too large' ? 413 : 400, {
      ok: false,
      error: error.message === 'Payload too large' ? 'Payload muito grande.' : 'Requisição inválida.',
    });
  }
});
